...
def train(model, train_iter, val_iter, test_iter,class_sentiment,class_intention,class_offensiveness,class_m_occurrence,class_m_category,is_meta):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            text = trains[0].to(DEVICE) # 32*20  [48, 768])
            meta = trains[1].to(DEVICE)  # 32*20  [48, 768])
            meta_pad = trains[2].to(DEVICE)  # 32*20  [48, 768])

            image = trains[3].to(DEVICE)   # [48, 1, 4096]
            id = trains[4]
            ...
            outputs_sentiment, outputs_intention, outputs_offensiveness, outputs_m_occurrence,outputs_m_occurrence_real = model(text,meta,meta_pad, image,source,target,text_mate)  # 前向传播 获取输出
            model.zero_grad()

            loss_1 = loss_func(outputs_sentiment, label_sentiment.long())  # 计算交叉熵损失
            loss_2 = loss_func(outputs_intention, label_intention.long())  # 计算交叉熵损失
            loss_3 = loss_func(outputs_offensiveness, label_offensiveness.long())  # 计算交叉熵损失
            loss_4 = loss_func(outputs_m_occurrence, label_m_occurrence.long())  # 计算交叉熵损失
            loss_5 = loss_func(outputs_m_occurrence_real, label_m_occurrence.long())  # 计算交叉熵损失
            ...
            if total_batch % 100 == 0:
                ...
                train_acc_sentiment = metrics.accuracy_score(label_sentiment.cpu(), predic_sentiment)  # 当前batch上训练集的准确率 因为是类别均衡数据集，所以可以直接用准确率作评估指标 不然使用 macro f1-score
                ...
                dev_acc_list,dev_pre_list,dev_rec_list, dev_loss_list = evaluate(model, val_iter,
                                                                 ...
                                                                 class_m_occurrence=class_m_occurrence,is_meta=is_meta)#计算此时模型在验证集上的损失和准确率
    ...
    test(model, test_iter, val=False,class_sentiment=class_sentiment,
                                                                     class_intention=class_intention,
                                                                     class_offensiveness=class_offensiveness,
                                                                     class_m_occurrence=class_m_occurrence,is_meta=is_meta)  # 模型训练结束后 进行测试
...
def test(model, test_iter, val=False,class_sentiment=7,class_intention=5,class_offensiveness=4,class_m_occurrence=2,class_m_category=3,is_meta=None):
    ...
    test_acc_list,test_pre_list,test_rec_list, test_loss_list, test_report_list, test_confusion_list = evaluate(model, test_iter, test=True,
                                                                class_sentiment=class_sentiment,
                                                                 class_intention=class_intention,
                                                                 class_offensiveness=class_offensiveness,
                                                                 class_m_occurrence=class_m_occurrence,is_meta=is_meta)  # 计算测试集准确率，每个batch的平均损失分类报告、混淆矩阵
    if val:
...
def evaluate(model, data_iter, test=False,class_sentiment=7,class_intention=5,class_offensiveness=4,class_m_occurrence=2,class_m_category=3,is_meta=None):
    ...
    with torch.no_grad():
        for (tests, labels) in data_iter:
            text = tests[0].to(DEVICE)  # 32*20  [48, 768])
            meta = tests[1].to(DEVICE)  # 32*20  [48, 768])
            meta_pad = tests[2].to(DEVICE)

            image = tests[3].to(DEVICE)  # [48, 1, 4096]
            ...
            label_offensiveness = labels[2].to(DEVICE)
            label_m_occurrence = labels[3].to(DEVICE)
            outputs_sentiment, outputs_intention, outputs_offensiveness, outputs_m_occurrence,outputs_m_occurrence_real = model(text, meta, meta_pad,image,source,target,text_mate)
